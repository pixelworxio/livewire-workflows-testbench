                ...
                <div class="p-8 text-gray-900">
                    ...
                    @if(session('appointment_success'))
                        <div class="bg-green-50 border border-green-200 rounded-lg p-4 mb-6">
                            <p class="text-green-800">{{ session('appointment_success') }}</p>
                        </div>
                    @endif
                    ...
                    @if(session('appointment_id'))
                        @php
                            $appointment = \App\Models\Appointment::with(['service', 'provider'])->find(session('appointment_id'));
                        @endphp
                            ...
                            <div class="bg-blue-50 border border-blue-200 rounded-lg p-6 mb-6">
                                ...
                                <div class="space-y-3">
                                    <div class="flex justify-between items-start">
                                        ...
                                        <span class="text-gray-900 font-semibold text-right">{{ $appointment->service->name }}</span>
                                    </div>

                                    <div class="flex justify-between items-start">
                                        <span class="text-gray-600 font-medium">Provider:</span>
                                        <span class="text-gray-900 font-semibold text-right">{{ $appointment->provider->name }}</span>
                                    </div>
                                    ...
                                    @if($appointment->notes)
                                        <div class="border-t border-blue-200 pt-3 mt-3">
                                            <span class="text-gray-600 font-medium block mb-2">Your Notes:</span>
                                            <p class="text-gray-900 text-sm">{{ $appointment->notes }}</p>
                                        </div>
                                    @endif
                                </div>
                            </div>
                    ...
                    <div class="flex flex-col sm:flex-row gap-4 justify-center">
                        ...
                        <a href="{{ route('appointment.start') }}" class="inline-flex items-center justify-center px-6 py-3 bg-white border border-gray-300 rounded-md font-semibold text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition">
